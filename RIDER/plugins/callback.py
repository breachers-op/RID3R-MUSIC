...
from RIDER import YouTube, app
from RIDER.core.call import RIDER
from RIDER.misc import SUDOERS, db
from RIDER.utils import bot_sys_stats
from RIDER.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from RIDER.utils.decorators.language import languageCB
from RIDER.utils.formatters import seconds_to_min
from RIDER.utils.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
from RIDER.utils.stream.autoclear import auto_clean
from RIDER.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await RIDER.pause_stream(chat_id)
        ...
        await RIDER.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await CallbackQuery.answer()
        await RIDER.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await RIDER.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await RIDER.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await RIDER.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
